...
const EnergySource = ({ type, value, position, day }) => {
  const icons = {
    type: '',
    arrow: 'fa-square',
    color: '#ffffff',
  ...
  const arrow = `fa-4x ${
    (value > 0 && position === 'left') || (value < 0 && position === 'right')
      ? 'fa-chevron-right'
      : 'fa-chevron-left'
  }`;
