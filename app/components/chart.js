...
export default class Chart extends Component {
  ...
  draw() {
    ...
    const line = d3
      ...
      .x(d => x(d.timestamp))
      .y(d => y(d.value));
    const inData = this.props.charts.in.map(({ value, timestamp }) => ({
      value,
      timestamp: d3Date(timestamp),
    }));
    const outData = this.props.charts.out.map(({ value, timestamp }) => ({
      value,
      timestamp: d3Date(timestamp),
    }));
    ...
    const startDate =
      Math.min(startInDate, startOutDate) || startInDate || startOutDate;
